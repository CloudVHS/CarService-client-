...
> В докладе/курсовой каждому экрану соответствует несколько исходных скриншотов (например, `login_1/2/3.png`, `main_list_1/2/3.png`, `car_detail_1/2.png`) — для README достаточно взять по одному репрезентативному изображению на экран, остальные можно оставить в папке `screenshots/` как дополнительные ракурсы или разместить в отдельной галерее.
